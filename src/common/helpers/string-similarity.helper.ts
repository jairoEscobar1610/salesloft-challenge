...
export function jaroWrinkerTest(s1: string, s2: string): number {
    ...
    let maxDistance: number = (Math.floor(Math.max(s1.length, s2.length) / 2)) - 1;
    ...
    for (let i = 0; i < s1.length; i++) {
        var low = (i >= maxDistance) ? i - maxDistance : 0,
            high = (i + maxDistance <= s2.length) ? (i + maxDistance) : (s2.length - 1);
...
export function stringSimilarityArray(contentArray: any[],
    ...
    return new Promise((resolve, reject) => {
        try {
            let resultSet : any = {};
            resultSet.groups = [];
            let stringArray = contentArray
                //Clear the data
                .map(mapFunction);
            //Fill hash array
            let hashArray = [], hasDuplicate = false, currentGroup = [];
            ...
            stringArray.forEach((currentText, index) => {
                ...
                if(!hashArray[index]){
                    ...
                    for (let j = index + 1; j < stringArray.length; j++) {
                        //It the string has not been marked yet
                        if(!hashArray[j]){
